...
REQUIRED_ITEM_MAX = player.PARTY_MATCH_REQUIRED_ITEM_MAX
...
class PartyMatch(ui.ScriptWindow):		
	...
	def __LoadWindow(self):
		...
		if not player.IsPartyMatchLoaded():
			return
	...
	def __CreateDungeonNameButton(self):
		...
		self.dungeon_dict = player.GetPartyMatchInfoMap()
	...
	def __ClickMatchingButton(self):
		...
		if 0 == self.selected_dungeon:
			self.__PartyMatchMsg((player.PARTY_MATCH_FAIL_NONE_MAP_INDEX, 0))
			return
		...
		if MATCH_STATE_SEARCHING == self.match_state:
			...
			## 파티매칭 취소 패킷을 날린다.
			net.SendPartyMatchCancel( self.selected_dungeon )
			## 신청한 시간 저장
		...
		if MATCH_STATE_NONE == self.match_state:
			...
			if player.GetStatus(player.LEVEL) < limit_level:
				self.__PartyMatchMsg((player.PARTY_MATCH_FAIL_LEVEL, 0))
				return
			...
			net.SendPartyMatchSearch( self.selected_dungeon )
	...
	def PartyMatchResult(self, type, data):
		if player.PARTY_MATCH_CANCEL == type:
			self.__PartyMatchMsg(data)
			self.__PartyMatchMinimapButton(data)
		elif player.PARTY_MATCH_INFO == type:
			self.__SetInfo(data)
			self.Close()
			if localeInfo.MINIMAP_ZONE_NAME_DICT_BY_IDX.has_key( data ):
				chat.AppendChat( chat.CHAT_TYPE_INFO, localeInfo.PARTY_MATCH_START % localeInfo.MINIMAP_ZONE_NAME_DICT_BY_IDX[data] )
	...
	def __PartyMatchMsg(self, data):
		...
		if player.PARTY_MATCH_FAIL == msg_type:
			pass
		elif player.PARTY_MATCH_SUCCESS == msg_type:
			# 매칭 성공
			pass
		elif player.PARTY_MATCH_CANCEL_SUCCESS == msg_type:
			# 파티매치 신청 취소
			if localeInfo.MINIMAP_ZONE_NAME_DICT_BY_IDX.has_key( add_info ):
				chat.AppendChat( chat.CHAT_TYPE_INFO, localeInfo.PARTY_MATCH_CANCEL % localeInfo.MINIMAP_ZONE_NAME_DICT_BY_IDX[add_info] )
		elif player.PARTY_MATCH_FAIL_NO_ITEM == msg_type:
			# 아이템이 부족
			item.SelectItem( add_info )
			itemName = item.GetItemName()
			chat.AppendChat( chat.CHAT_TYPE_INFO, localeInfo.PARTY_MATCH_FAIL_NO_ITEM % itemName )
		elif player.PARTY_MATCH_FAIL_LEVEL == msg_type:
			# 레벨이 낮음
			chat.AppendChat( chat.CHAT_TYPE_INFO, localeInfo.PARTY_MATCH_FAIL_LEVEL )
		elif player.PARTY_MATCH_FAIL_NOT_LEADER == msg_type:
			# 파티장이 아님
			chat.AppendChat( chat.CHAT_TYPE_INFO, localeInfo.PARTY_MATCH_FAIL_NOT_LEADER )
		elif player.PARTY_MATCH_FAIL_NONE_MAP_INDEX == msg_type:
			# 파티매칭에 없는 맵번호일때(혹은 선택한 던전이 없을때)
			chat.AppendChat( chat.CHAT_TYPE_INFO, localeInfo.PARTY_MATCH_FAIL_NONE_MAP_INDEX )
		elif player.PARTY_MATCH_FAIL_IMPOSSIBLE_MAP == msg_type:
			# 현재 신청 불가능한 맵에 있을때
			chat.AppendChat( chat.CHAT_TYPE_INFO, localeInfo.PARTY_MATCH_FAIL_IMPOSSIBLE_MAP )
		elif player.PARTY_MATCH_HOLD == msg_type:
			return
	...
	def __PartyMatchMinimapButton(self, data):
		...
		(msg_type, add_info) = data
		if player.PARTY_MATCH_START == msg_type:
			self.minimap.ShowPartyMatchButton()
		elif msg_type in [player.PARTY_MATCH_CANCEL_SUCCESS, player.PARTY_MATCH_SUCCESS, player.PARTY_MATCH_FAIL]:
